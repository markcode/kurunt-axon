...
ReqSocket.prototype.onmessage = function(){
  ...
  return function(msg, multipart){
    ...
    //console.log('REPlied can remove this msg from commits q');
    if ( self.settings.guarantee_delivery ) {
      self.committed(msg);  // markcode: check commits and remove this message, committed.
    }
    fn.apply(null, msg);
...
ReqSocket.prototype.send = function(msg){
  ...
  if (sock && this.settings.guarantee_delivery) {
    args[0] = this.commit(sock, args);  // markcode: commit this message for guaranteed delivery.
  }
  
  if (sock) {
    // TODO, if sending queued messages, should batch and round-robin, so not all get sent to same downstream.
    sock.write(this.pack(args));
    this.commitMsgArraytoObj(args);
  } else {
